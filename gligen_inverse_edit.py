...
def prepare_batch(meta, batch=1, max_objs=30):
    ...
    ori_phrases, edit_phrases, images = meta.get("ori_phrases"), meta.get("edit_phrases"), meta.get("images")
    images = [None]*max(len(ori_phrases), len(edit_phrases)) if images==None else images
    ori_phrases = [None]*len(images) if ori_phrases==None else ori_phrases
    edit_phrases = [None]*len(images) if edit_phrases==None else edit_phrases
    max_objs = min(len(ori_phrases), max_objs)

    version = "openai/clip-vit-large-patch14"
    model = CLIPModel.from_pretrained(version).cuda()
    processor = CLIPProcessor.from_pretrained(version)

    def prepare_per_batch(meta, phrases, images):
    ...
    return prepare_per_batch(meta, ori_phrases, images), prepare_per_batch(meta, edit_phrases, images)
...
def run(meta, config, starting_noise=None):
    ...
    if "keypoint" in meta["ckpt"]:
        ...
        ori_batch, target_batch = batch = prepare_batch(meta, config.batch_size) # impainting mode using only one box
    ...
    if "input_image" in meta:
        ...
        if meta["num_masks"] > 0:
            total_mask = read_masks_from_figure( meta["input_masks"], meta["num_masks"], model.image_size ).to(device)
            inpainting_mask_list = []
            for i in range(args.batch_size):
                inpainting_mask_list.append( total_mask )
            inpainting_mask = torch.stack(inpainting_mask_list).unsqueeze(1).to(device)
            inpainting_mask_ori = draw_masks_from_boxes( batch['boxes'], model.image_size  ).to(device)
        else:
    ...
    ori_grounding_extra_input, target_grounding_extra_input = None, None
    if grounding_downsampler_input != None:
        ori_grounding_extra_input = grounding_downsampler_input.prepare(ori_batch)
        target_grounding_input = grounding_downsampler_input.prepare(target_batch)

    # - - DDIM Inversion - - #
    # import pdb; pdb.set_trace()
    # null_inversion = NullInversion(model, autoencoder, text_encoder, diffusion, config)

    starting_noise = starting_noise if starting_noise is not None else torch.randn(config.batch_size, 4, 64, 64)
    ...
    ori_input = dict(
                ...
                timesteps = None,
                context = ori_context,
                grounding_input = ori_grounding_input,
                inpainting_extra_input = inpainting_extra_input,
                grounding_extra_input = ori_grounding_extra_input,
    ...
    target_input = dict(
                ...
                grounding_input = target_grounding_input,
                inpainting_extra_input = inpainting_extra_input,
                grounding_extra_input = target_grounding_extra_input,
    ...
    print('DDIM original generation starts')

    ori_samples_fake_z = sampler.sample(S=steps, shape=shape, input=ori_input,  uc=uc, guidance_scale=config.guidance_scale, mask=inpainting_mask, x0=z0)
    target_samples_fake_z = sampler.sample(S=steps, shape=shape, input=target_input,  uc=uc, guidance_scale=config.guidance_scale, mask=inpainting_mask, x0=z0)
    ori_samples_fake = autoencoder.decode(ori_samples_fake_z)
    target_samples_fake = autoencoder.decode(target_samples_fake_z)

    # - - - - - DDIM Inversion - - - - - #
    ori_input["x"] = ori_samples_fake_z
    ori_inverse_z = sampler.inverse(S=steps, shape=shape, input=ori_input, uc=uc, guidance_scale=config.guidance_scale, mask=inpainting_mask, x0=z0)
    ori_input["x"] = ori_inverse_z
    ori_inverse_recons_z = sampler.sample(S=steps, shape=shape, input=ori_input,  uc=uc, guidance_scale=config.guidance_scale, mask=inpainting_mask, x0=z0)
    ori_samples_DDIMinverse = autoencoder.decode(ori_inverse_recons_z)
    target_input["x"] = ori_inverse_z
    target_inverse_recons_z = sampler.sample(S=steps, shape=shape, input=target_input,  uc=uc, guidance_scale=config.guidance_scale, mask=inpainting_mask, x0=z0)
    target_samples_DDIMinverse = autoencoder.decode(target_inverse_recons_z)
    ...
    def save_img(image_ids, images, type=''):
        for image_id, sample in zip(image_ids, images):
            img_name = str(int(image_id))+type+'.png'
            sample = torch.clamp(sample, min=-1, max=1) * 0.5 + 0.5
            sample = sample.cpu().numpy().transpose(1,2,0) * 255
            sample = Image.fromarray(sample.astype(np.uint8))
            sample.save(  os.path.join(output_folder, img_name)   )

    save_img(image_ids, ori_samples_fake)
    save_img(image_ids, ori_samples_DDIMinverse, type="R")
    save_img(image_ids, target_samples_fake, type="D")
    save_img(image_ids, target_samples_DDIMinverse, type="E")
...
if __name__ == "__main__":
    ...
    # meta =  dict(
    #         ckpt = "gligen_checkpoints/checkpoint_inpainting_text.bin",
    #         input_image = "inference_images/beach.jpg",
    #         input_masks = "inference_images/beach_mask",
    #         input_prompt = "flower",
    #         prompt = "a lamp",
    #         phrases =   ['lamp'],
    #         locations = [ [0.26, 0.31, 0.74, 0.72] ],
    #         # locations = [ [0.41, 0.48, 0.72, 0.74] ],
    ...
    meta = dict(
            ...
            ori_prompt = "a man hugging a woman on the beach",
            edit_prompt = "a girl with a hat hugging a woman on the beach",
            ori_phrases = ['man'],
            edit_phrases = ['girl'],
            locations = [],
